import Image from 'next/image'
import Navbar from '@/components/navbar'
import Footer from '@/components/footer'
import Link from 'next/link'

const trainingRoomOffers = [
  { id: 1, name: '2 Hours', price: 50 },
  { id: 2, name: '4 Hours', price: 60 },
  { id: 3, name: 'Full Day', price: 100 }
]

export default function TrainingRoomPage() {
  const offers = trainingRoomOffers

  return (
    <div className="min-h-screen">
      <Navbar />

      <main className="bg-gradient-to-br from-[#334247] to-[#4f8fa3] pt-20 pb-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          {/* Header */}
          <Link href="/services" className="text-[#b4c7d4] hover:text-white mb-8 inline-block transition-colors">
            ← Back to Services
          </Link>

          <h1 className="text-4xl md:text-5xl font-bold text-white mb-6">Conference Rooms</h1>
          <p className="text-lg text-[#b4c7d4] mb-12">Fully equipped training and conference spaces perfect for workshops, seminars, and events</p>

          <div className="relative w-full h-96 bg-gray-200 rounded-lg mb-12 flex items-center justify-center overflow-hidden">
            <Image
              src="/training-room.png"
              alt="Conference Rooms"
              fill
              sizes="100vw"
              className="object-cover"
            />
          </div>

          {/* Pricing Section */}
          <div className="mb-12">
            <h2 className="text-3xl font-bold text-white mb-8">Our Offers</h2>

            {/* Scrollable Pricing Table */}
            <div className="overflow-x-auto rounded-lg border border-border">
              <div className="inline-flex gap-4 p-6 min-w-full">
                {offers.map((offer) => (
                  <div key={offer.id} className="flex-shrink-0 w-56 bg-white rounded-lg border border-[#b4c7d4]/30 p-6 shadow-sm hover:shadow-md transition-shadow">
                    <h3 className="text-lg font-semibold text-[#334247] mb-4">{offer.name}</h3>

                    <div className="space-y-3">
                      <p className="text-3xl font-bold text-[#334247]">{offer.price} <span className="text-lg">Dt</span></p>
                    </div>
                  </div>
                ))}
              </div>
            </div>
          </div>

          {/* Description */}
          <div className="bg-white/10 backdrop-blur-md border border-[#b4c7d4]/30 rounded-lg p-8">
            <h3 className="text-2xl font-bold text-white mb-4">Training Room Features</h3>
            <ul className="space-y-3 text-[#b4c7d4]">
              <li className="flex items-start gap-3">
                <span className="text-[#b4c7d4] font-bold">✓</span>
                <span>Spacious room with flexible layout options</span>
              </li>
              <li className="flex items-start gap-3">
                <span className="text-primary font-bold">✓</span>
                <span>Projector, screen, and audio system included</span>
              </li>
              <li className="flex items-start gap-3">
                <span className="text-primary font-bold">✓</span>
                <span>Capacity for up to 30 participants</span>
              </li>
              <li className="flex items-start gap-3">
                <span className="text-primary font-bold">✓</span>
                <span>Optional catering and break room services</span>
              </li>
            </ul>
          </div>
        </div>
      </main>

      <Footer />
    </div>
  )
}
